...
In everyday life, whenever we write a name, a sentence, or even a password — that's a string. In C programming, a **string is simply a sequence of characters stored together**. For example, the word *"Hello"* is a string made up of 5 characters: H, e, l, l, o.
...
This is where the magic happens. In C, every string ends with a special character called the **null character**, written as `\0`. This tells the computer: *"Hey, the string ends here."* Without it, C wouldn't know where the string stops in memory.

So in memory, *"Hello"* is actually stored as: **H, e, l, l, o, \0** — that's 6 characters total.
...
But there's a catch — `scanf` **stops reading at a space**. So if you type *"John Doe"*, it will only store *"John"*.
...
These four functions are the ones you'll use most frequently in C string programming.
